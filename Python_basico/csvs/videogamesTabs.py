...
def write_csv(file_path, data):
    with open(file_path, 'w', encoding='utf-8', newline='') as file:
        headers = data[0].keys()
        writer = csv.DictWriter(file, fieldnames=headers, delimiter='\t') # el separador es una tabulador en lugar de uan coma
        writer.writeheader()
...
def main():
    try:
        ...
        write_csv("favoritesVideogamesTabs.csv", videogames_dict)
    except ValueError as ex:
        print(ex)
